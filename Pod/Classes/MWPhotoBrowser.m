...
- (void)viewDidLoad {
	...
	{
		_toolbar.tintColor = [self.view tintColor];
	}
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), self.customHeaderWordOf ?: NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
	} else {
		self.title = nil;
	}
